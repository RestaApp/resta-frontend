...
function SearchFiltersComponent({ activeFiltersList, onResetFilters }: SearchFiltersProps) {
  ...
  return (
    <div className="ui-density-page flex items-center gap-1 border-b border-border/50 bg-background py-1">
      <div className="flex min-w-0 flex-1 gap-1 overflow-x-auto scrollbar-hide">
        {activeFiltersList.map((filter, index) => (
          <Badge
            key={index}
            variant="tag"
            className="shrink-0 whitespace-nowrap px-2 py-0 text-xs leading-tight"
          >
            {filter}
          </Badge>
        ))}
