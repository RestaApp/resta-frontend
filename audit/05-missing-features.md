...
| Авто-закрытие `review_reminder` | 🟡 | ❌ | **P1** | После оставленного отзыва напоминание «Оставьте отзыв» не закрывается — нет связи `Review.create` ↔ `Notification` (= C14). Висит `read` до 30-дневной чистки; UI приглашает оставить уже оставленный отзыв. Фикс: бэк архивирует напоминание (A), FE инвалидирует тег (B10) |
