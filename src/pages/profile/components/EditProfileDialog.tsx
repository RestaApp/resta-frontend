...
export function EditProfileDialog({ open, onOpenChange, onSuccess }: EditProfileDialogProps) {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            if (Object.keys(updateData.user).length === 0 && !updateData.user.employee_profile_attributes) {
                onOpenChange(false)
                return
            }
                        ...
                        {isEmployee && (
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    className="w-4 h-4 rounded border-border text-primary focus:ring-2 focus:ring-primary"
                                />
                                <label htmlFor="open-to-work-checkbox" className="text-sm font-medium cursor-pointer">
                                    Открыт к работе
                                </label>
                            </div>
                        )}
