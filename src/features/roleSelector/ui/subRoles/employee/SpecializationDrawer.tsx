...
import { ExperienceField, LocationField, OpenToWorkToggle } from './fields'
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className={`space-y-5 ${specializations.length > 0 ? 'pt-4 border-t border-border' : ''}`}>
              <ExperienceField value={formData.experienceYears} onChange={(v) => onFormDataUpdate({ experienceYears: v })} />
              <OpenToWorkToggle value={formData.openToWork} onChange={(v) => onFormDataUpdate({ openToWork: v })} />
              <LocationField
                value={formData.location}
                onChange={(v) => onFormDataUpdate({ location: v })}
