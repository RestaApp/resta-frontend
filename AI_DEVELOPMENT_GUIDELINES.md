...
```css
.pb-safe-nav   /* bottom nav / fixed нижняя навигация — safe-area + 12px */
```
...
- **Safe‑area:** `fixed bottom-0` → утилита `pb-safe-nav`. Ручные `pb-[calc(...env(safe-area)...)]` запрещены.
...
- **Scroll jank:** `* { transition: all }` запрещено. Theme transitions — только на `body`.
...
- **`vite.config.ts` — без `manualChunks`** (канон): не разносить `react`,
  `react-redux`, `redux-persist`, `motion` по отдельным vendor-чанкам — циклические
  импорты → `TypeError: undefined is not an object (evaluating 'd.Component')` в
  Safari / Telegram WebView. Детали — `.cursor/rules/vite-build-chunking.mdc`.
...
- `--gradient-stars` и `text-warning` — premium/звёзды; базовый amber — `--warning`.
