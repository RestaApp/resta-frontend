/**
 * Экран формы для поставщика с полями: название компании, категория товаров, город
 */
...
interface SupplierFormScreenProps {
    ...
    onFormDataUpdate: (updates: Partial<SupplierFormData>) => void
...
export const SupplierFormScreen = memo(function SupplierFormScreen({
    supplierTypes,
    formData,
    onFormDataUpdate,
    onLocationRequest,
...
}: SupplierFormScreenProps): JSX.Element {
    useEffect(() => {
        ...
        <div className="min-h-screen bg-background flex flex-col px-6 py-12">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {supplierTypes.map(type => (
                            <SelectableTagButton
                                ...
                                ariaLabel={`Выбрать: ${getSupplierTypeLabel(type)}`}
                            />
                        ))}
                    </div>
                </motion.div>
            ...
            <div className="max-w-md mx-auto w-full mt-8 pb-8">
                <Button
                    ...
                    size="lg"
                    aria-label="Продолжить заполнение формы поставщика"
                >
                    Продолжить
                </Button>
