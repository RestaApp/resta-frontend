  ...
  const content = (
    <>
      <label className={`block ${withAnimation ? 'mb-3' : 'mb-2'} text-foreground font-medium text-sm`}>
        Опыт работы: {value === 5 ? '5+' : `${value} ${getYearsLabel(value)}`}
      </label>
      <input
        type="range"
        min="0"
    ...
    <>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-4 py-3 rounded-2xl border-2 border-border hover:border-primary/50 transition-colors flex items-center justify-center"
