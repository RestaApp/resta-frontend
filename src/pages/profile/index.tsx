...
export function ProfileScreen({ onNavigate, onBack, activeTab, onTabChange }: ProfileScreenProps) {
  ...
  const profile = {
    name:
      ...
      userProfile?.name ||
      (isRestaurant ? 'Ресторан' : isSupplier ? 'Поставщик' : 'Пользователь'),
    ...
    experience: userProfile?.employee_profile?.experience_years
      ? `${userProfile.employee_profile.experience_years} ${userProfile.employee_profile.experience_years === 1 ? 'год' : userProfile.employee_profile.experience_years < 5 ? 'года' : 'лет'}`
      : 'Не указано',
    certificates:
      ...
      <div className="px-4 py-6 space-y-5">
        ...
        <Card className="p-6 shadow-md border border-border/50 bg-gradient-to-br from-card via-card to-card/95 hover:shadow-lg transition-all duration-300">
          <div className="flex items-start gap-5 mb-6">
            ...
            <div className="flex-1 min-w-0 pt-1">
              <h1 className="text-2xl font-bold mb-3 text-foreground leading-tight tracking-tight">
                {profile.name}
              </h1>
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                {profile.position && (
                  <Badge
                    variant="outline"
                    className="text-xs px-3 py-1.5 border-primary/40 bg-primary/10 hover:bg-primary/15 transition-colors"
                  >
                  ...
                  <>
                    {profile.specializations.map((spec: string, index: number) => (
                      <Badge
                        key={index}
                        variant="outline"
                        className="text-xs px-3 py-1.5 border-border/60 bg-muted/50 hover:bg-muted transition-colors"
                      >
                        {spec}
                      </Badge>
                    ))}
              ...
              <div className="flex items-center gap-3 text-sm">
                <div className="flex items-center gap-1.5">
                  <Star
                    className={`w-4 h-4 transition-colors ${profile.rating > 0 ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground'}`}
                  />
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card className="p-5 text-center border border-border/50 hover:border-primary/40 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-card via-card to-primary/5 hover:scale-[1.02] active:scale-[0.98] cursor-default">
            <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-primary/20 via-primary/15 to-primary/10 flex items-center justify-center shadow-md ring-2 ring-primary/5">
              <Star
                className={`w-6 h-6 transition-colors ${profile.rating > 0 ? 'text-yellow-400 fill-yellow-400' : 'text-primary'}`}
                strokeWidth={2}
              />
            </div>
            <div className="text-3xl font-bold text-foreground mb-2 leading-tight min-h-[36px] flex items-center justify-center">
        ...
        {(profile.email || profile.phone || profile.location) && (
          <Card className="p-6 border border-border/50 shadow-sm hover:shadow-md transition-all duration-300">
            <h2 className="text-lg font-bold mb-5 text-foreground tracking-tight">
            ...
            <div className="flex flex-wrap gap-2.5">
              {profile.skills.map((skill: string, index: number) => (
                <Badge
                  key={index}
                  variant="outline"
                  className="px-3.5 py-2 text-sm border-primary/40 bg-primary/10 hover:bg-primary/15 hover:border-primary/50 transition-all duration-200"
                >
                  {skill}
                </Badge>
              ))}
            ...
            {profile.certificates.length > 0 ? (
              profile.certificates.map((cert: string, index: number) => (
                <Badge
                  key={index}
                  variant="outline"
                  className="gap-2 px-3.5 py-2 border-primary/40 bg-primary/10 hover:bg-primary/15 hover:border-primary/50 transition-all duration-200"
                >
            ...
            return (
              <div key={index}>
                ...
                >
                  ...
                  <span className="flex-1 text-left text-sm font-semibold text-foreground group-hover:text-primary transition-colors duration-200">
                    {item.label}
                  </span>
                  <span className="text-muted-foreground/60 group-hover:text-primary group-hover:translate-x-0.5 transition-all duration-200 text-lg font-light">
                    ›
                  </span>
                </button>
        ...
        {isRestaurant && (
          <Card className="p-6 bg-gradient-to-br from-primary/15 via-primary/10 to-primary/5 border-2 border-primary/30 shadow-md hover:shadow-lg transition-all duration-300">
            ...
            <Button
              className="w-full h-12 font-bold shadow-md hover:shadow-lg active:scale-[0.98] transition-all duration-200"
              onClick={() => onNavigate(ROUTES.CREATE_SHIFT)}
            >
              Создать вакансию
            </Button>
          </Card>
      ...
      <AlertDialog open={showChangeRoleDialog} onOpenChange={setShowChangeRoleDialog}>
        <AlertDialogContent className="max-w-[340px] rounded-3xl shadow-2xl border-2">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              className="rounded-xl h-11 font-bold shadow-md hover:shadow-lg active:scale-[0.98] transition-all duration-200"
